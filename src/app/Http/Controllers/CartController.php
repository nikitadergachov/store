...
class CartController extends Controller {
    ...
    public function addCart() {
        ...
        if ($validator->fails()) {
            flash()->error('Ошибка', 'Товар не может быть добавлен в корзину!');
        ...
        $search_in_cart = Cart::where('user_id', '=', $user_id)->where('product_id', '=', $product_id)->get();
        if(!$search_in_cart->isEmpty()){
            flash()->error('Ошибка', 'Товар уже есть в корзине!');
            return redirect()->route('cart');
        }

        $not_available = Cart::where('user_id', '=', $user_id)->where('qty', '=', 0)->get();
        if(!$not_available->isEmpty()){
            flash()->error('Ошибка', 'Товара нет в наличии!');
            return redirect()->back();
        }
